...
function initSearch() {
  ...
  request.onload = function(){
    if (request.status >= 200 && request.status < 400) {
      ...
      lunr.tokenizer.separator = /[\s\-/]+|[\u4e00-\u9fa5]/; // 匹配空格、斜杠、中文字符

      var index = lunr(function(){
        this.use(lunr.zh); // 启用中文分词插件
        this.ref('id');
...
function searchLoaded(index, docs) {
  ...
  function update() {
    ...
    var results = index.query(function (query) {
      ...
      query.term(tokens, {
        boost: 10,
        usePipeline: true, // 启用中文分词管道
        wildcard: lunr.Query.wildcard.NONE // 禁用通配符（中文不适用）
      });
    });
    ...
    if ((results.length == 0) && (input.length > 2)) {
      ...
      if (tokens.length > 0) {
        results = index.query(function (query) {
          query.term(tokens, {
            editDistance: Math.min(2, Math.floor(input.length / 4))  // 限制最大容错为2
          });
        });
      }
    }
